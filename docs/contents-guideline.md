...
> **상태**: 확정 — v9.0
> 마지막 업데이트: 2026-04-25 (생성 파이프라인·Zone 컨텍스트·LLM 프롬프트 통합)
...
> Zone 정보는 **이미지 선택·인사말·테마 스코어링**에 활용됩니다.
>
> ⚠️ **v9.1 (2026-04-26) 변경**: 앱은 하루 1개 말씀을 Zone 무관하게 전체 풀에서 선택합니다.
> `application`은 **범용 작성**이 원칙입니다. Zone별 시간대 언급 금지. (§4-3 참고)
...
### 4-3. application 작성 원칙 (v9.1 — 범용)

> ⚠️ **v9.1 변경**: 앱은 하루 1개 말씀을 전체 유저에게 동일하게 제공합니다.
> 말씀이 어떤 시간대에도 자연스럽게 읽히도록 **범용 작성이 원칙**입니다.

#### 금지 표현 (시간대 언급)
```
❌ "알람 끄고 30초만 눈 감아봐"         → 아침에만 적합
❌ "이제 편히 자"                       → 취침 전에만 적합
❌ "지금 이 쉬는 시간"                  → 점심 시간대만 적합
❌ "퇴근길에", "출근하며", "아침에 일어나" → 특정 시간대 언급
```
...
| 기준 | 설명 |
|------|------|
| **보편적 상황** | 아침/낮/밤 어디서 읽어도 어색하지 않은 행동 제안 |
| **내면 지향** | 외부 행동보다 내면 태도·시선 변화 |
| **단순 행동** | 잠깐 멈추기, 기억하기, 말해보기, 내려놓기 등 |
| **말투** | ~해봐, ~기억해, ~말해봐, ~내려놔 (유지) |
| **금지** | 반드시, 꼭, ~해야 한다, 특정 시간대 언급 (유지) |

#### 좋은 예 vs 나쁜 예

```
✅ 범용: "오늘 두렵거나 막히는 게 있다면, 잠깐 멈추고 기억해봐. 혼자가 아니야."
✅ 범용: "지금 이 순간, 그분이 함께한다는 걸 한 번만 마음속으로 말해봐."
✅ 범용: "내가 통제할 수 없는 것들을 오늘 하루 내려놔봐."

❌ 시간대: "알람 끄고 30초만 눈 감아봐. 오늘도 혼자가 아님을 기억하며 시작해."
❌ 시간대: "알람을 맞추며 기억해. 내일 무슨 일이 생겨도 그분 손 안에 있어. 이제 편히 자."
```
...
| `theme` | String 배열 | 아래 테마 풀 참고 (17개) | 옵셔널 매칭 — 있으면 우선 노출, 없으면 랜덤 |
...
> 기존 12개 + 데이터에 실제 존재하며 AppMode.themes에 이미 사용되는 5개 추가

| 테마 | 의미 | 주요 Zone |
|------|------|---------|
| `hope` | 소망 | rise_ignite |
| `courage` | 용기 | rise_ignite, peak_mode |
| `strength` | 힘 | rise_ignite, peak_mode, second_wind |
| `renewal` | 새로움 | rise_ignite, first_light |
| `wisdom` | 지혜 | peak_mode, second_wind |
| `focus` | 집중 | peak_mode, second_wind |
| `patience` | 인내 | recharge, second_wind |
| `gratitude` | 감사 | recharge, golden_hour |
| `peace` | 평안 | golden_hour, wind_down |
| `comfort` | 위로 | recharge, golden_hour, wind_down |
| `reflection` | 성찰 | golden_hour |
| `rest` | 쉼 | recharge, wind_down |
| `faith` | 믿음 | deep_dark, first_light ← **v9.1 추가** |
| `grace` | 은혜 | deep_dark, first_light ← **v9.1 추가** |
| `trust` | 신뢰 | 전체 ← **v9.1 추가** |
| `stillness` | 고요함 | deep_dark, first_light, wind_down ← **v9.1 추가** |
| `surrender` | 내려놓음 | deep_dark, wind_down ← **v9.1 추가** |
...
#### 사용 순서
1. `{성경 구절 원문}`, `{참조}` 채우기
2. Claude에 전달 → JSON 출력 확인 → 자기검증 체크리스트 통과 확인
...
morning manna는 크리스천을 위한 iOS 알람 앱으로, 하루 1개 말씀을 전체 유저에게 동일하게 제공해.
글쓰기 스타일: 설교자가 아닌 유저의 신앙 친구. 교회 강단 언어 아님.
목표: 아침이든 낮이든 밤이든 언제 읽어도 자연스럽고 따뜻하게 닿는 말씀 전달.
...
[② verse_short_ko | 10~60자 | full에서 추출]
...
⚠️ v9.1 변경: Zone 시간대 언급 금지. 아침/낮/밤 어디서 읽어도 어색하지 않은 범용 문장.

규칙:
- 오늘 바로 실천 가능한 구체적 행동 1가지
- 보편적 상황 — 내면 태도·시선 변화 중심
- 말투: ~해봐, ~기억해, ~말해봐, ~생각해봐, ~내려놔
- 금지: 반드시, 꼭, ~해야 한다, 시간대 언급 (알람 끄고, 퇴근하며, 이제 편히 자 등)

✅ 좋은 예:
"오늘 두렵거나 막히는 게 있다면, 잠깐 멈추고 기억해봐. 혼자가 아니야."
"지금 이 순간, 그분이 함께한다는 걸 한 번만 마음속으로 말해봐."
"내가 통제할 수 없는 것들을 오늘 하루 내려놔봐."

❌ 나쁜 예:
"알람 끄고 30초만 눈 감아봐." → 시간대 언급
"알람을 맞추며 기억해. 이제 편히 자." → 시간대 언급
...
□ interpretation이 "①저자상황 → ②핵심의미 → ③오늘연결" 3단계 순서인가?
□ application에 시간대 언급이 없는가? (알람 끄고, 퇴근하며, 이제 편히 자 등)
...
| 컬럼명 | 기준 글자수 | 범위 | 비고 |
|--------|-----------|------|------|
| verse_full_ko | 40자 | 40~150자 | **앵커. 먼저 확정.** 구두점·줄바꿈 철저 |
| verse_short_ko | full에서 추출 | **10~60자** | full 확정 후 작성. 마침표·쉼표 철저 |
