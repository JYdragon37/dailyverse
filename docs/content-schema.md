...
| `verse_full_ko` | 20자 | 60자 | **150자** | 개역한글 원문, `\n`으로 줄 구분 |
| `interpretation` | 80자 | 120자 | 200자 | 3단계 구조 필수 |
| `application` | 40자 | 60자 | 100자 | 실천 1가지, 범용 (시간대 언급 금지) |
| `alarm_top_ko` | 10자 | 20자 | 35자 | 없으면 verse_short_ko 폴백 |
| `question` | 20자 | 50자 | 80자 | 질문형 1~2문장 |

### interpretation 3단계 구조 (v9.1 — Zone 맥락 제거)

```
① 배경/맥락 (1~2문장) — 저자, 역사적 배경, 대상
② 핵심 의미 (1~2문장) — 핵심 단어 뜻 (원어 직접 표기 금지, 한국어로 풀어서)
③ 오늘 연결 (1문장)   — 현대 삶과의 접점 ("지금 네가...", "이 말씀은 오늘 너에게...")
```

> ④ Zone 맥락 제거 이유: 앱은 하루 1개 말씀을 Zone 무관하게 전체 유저 동일 노출.
> 시간대 언급이 포함되면 잘못된 맥락으로 읽힘.
