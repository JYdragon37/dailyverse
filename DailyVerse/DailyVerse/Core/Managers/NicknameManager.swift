...
final class NicknameManager: ObservableObject {
    ...
    private init() {
        ...
        _nickname = Published(initialValue: UserDefaults.standard.string(forKey: Self.nicknameKey) ?? "stranger")
    }

    /// 유저가 명시적으로 닉네임을 설정했는지 여부
    /// - nicknameSet 플래그가 true이고 닉네임이 기본값("stranger")이 아닌 경우만 true
    var isSet: Bool {
        UserDefaults.standard.bool(forKey: Self.nicknameSetKey) && nickname != "stranger"
    }
    ...
    func setNickname(_ name: String, userId: String? = nil) async {
        ...
        nickname = trimmed.isEmpty ? "stranger" : String(trimmed.prefix(limit))
    ...
    func reset() {
        _nickname = Published(initialValue: "stranger")
    ...
    func syncWithFirestore(userId: String) async {
        if let user = try? await firestoreService.fetchUser(uid: userId),
           !user.nickname.isEmpty, user.nickname != "stranger" {
            // Firestore 값이 있으면 → 로컬 업데이트
            nickname = user.nickname
            UserDefaults.standard.set(nickname, forKey: Self.nicknameKey)
        }
        // else: 신규 유저면 로컬 "stranger"(reset 후 상태) 유지, Firestore 저장 안 함
