 ...
 *   - Firebase Storage 업로드 → VERSE_IMAGES 시트 먼저 추가 → Firestore 저장
 *
 * 사용법:
 *   node upload_verse_images.js
 */
...
async function main() {
  ...
  for (const filePath of allFiles) {
    ...
    // 1. Sheets에 먼저 추가 (Single Source of Truth)
    const row = [
    ...
    await sheets.spreadsheets.values.append({
      ...
      range: 'VERSE_IMAGES!A1',
      valueInputOption: 'RAW',
      insertDataOption: 'INSERT_ROWS',
      requestBody: { values: [row] },
    });
    console.log(`   📊 VERSE_IMAGES 시트 추가 완료`);

    // 2. Firestore 저장 (Sheets 성공 후)
